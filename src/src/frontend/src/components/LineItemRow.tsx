...
export default function LineItemRow({
  ...
  const suggestions =
    lineItem.itemName.trim().length > 0
      ? savedItems
          .filter(
            (i) =>
              i.itemName
                .toLowerCase()
                .includes(lineItem.itemName.toLowerCase()) ||
              i.itemCode
                .toLowerCase()
                .includes(lineItem.itemName.toLowerCase()),
          )
          ...
          {suggestionOpen && suggestions.length > 0 && (
            ...
            >
              {suggestions.map((item, idx) => (
                ...
                >
                  <span className="font-mono text-[10px] bg-accent text-accent-foreground px-1.5 py-0.5 rounded shrink-0 leading-tight">
                    {item.itemCode || "—"}
                  </span>
